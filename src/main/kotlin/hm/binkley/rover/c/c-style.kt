    ...
    while (i < l) {
        ...
        if (maxX < x) {
            throw IllegalArgumentException(
                "Line #${co.index}: Malformed input: ${co.value}:" +
                    " Out of bounds: ($maxX, $maxY) < ($x, $y)"
            )
        }
        if (maxY < y) {
            throw IllegalArgumentException(
        ...
        while (j < k) {
            when (ins[j]) {
                ...
                'B' -> when (d % 4) {
                    ...
                    "Line #${mo.index}: Malformed input: $ins:" +
                        " Not an instruction: ${ins[j]}"
                )
            }
...
private fun parseBoundaries(where: IndexedValue<String>): List<Int> {
    ...
    return when (boundaries.size) {
        2 -> {
            ...
            "Line #${where.index}: Malformed input: ${where.value}:" +
                " Boundary must have 2 elements"
        )
    }
}
